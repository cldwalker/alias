module Alias
  class DelegateToClassMethodCreator < Creator
    ...
    valid :aliased_method, :unless=>lambda {|e| (klass = Util.any_const_get(e[:class])) && klass.method_defined?(e[:alias]) },
      :message=>"%klass: alias to method '%aliased_method' deleted since it already exists"
    valid :delegate_method, :if=>lambda {|e| (klass = Util.any_const_get(e[:delegate_class])) && klass.respond_to?(e[:delegate_name]) },
